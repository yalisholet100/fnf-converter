(function ($global) {
            ...
            switch (notevalue) {
              ...
              case "3":
                ...
                if (holdhead != null) {
                  ...
                  console.log(
                    "SMChart.hx:72:",
                    "[ERROR] Encountered tail " + notevalue + " with no head!"
                  );
                }
                break;
              case "2":
              case "4":
                ...
                holdtracker.h[columnIndex] =
                  latestSection.sectionNotes[
                    latestSection.sectionNotes.length - 1
                  ];
                break;
            }
          }
  ...
  SMFile.prototype = {
    ...
    makeFNFChart: function (chartIndex, song_config, flipchart) {
      ...
      var fnfjson = {
        song: val == null ? this.extraHeaderTags.h["TITLE"] : val,
        notes: [],
        events: [],
        bpm: this.bpms[0][1],
        needsVoices: false,
        speed: val1 == null ? 1.0 : val1,
        player1: val2 == null ? "bf" : val2,
        player2: val3 == null ? "dad" : val3,
        gfVersion: val4 == null ? "gf" : val4,
        stage: "",
        validScore: true,
      };
      var fnfchart = this.charts[chartIndex].toFNF(
        this.bpms,
        this.chartOffset,
        flipchart
      );
  ...
  WebMain.parseSMFile = function (smString) {
    ...
    while (WebMain.chartselect.children.length > 0)
      WebMain.chartselect.remove(0);
    var _g = 0;
  ...
  WebMain.generateChart = function (e) {
    var cfg = {
      song:
        WebMain.songnameinput.value != ""
          ? WebMain.songnameinput.value
          : "SMSong",
      speed:
        WebMain.songspeedinput.value != ""
          ? parseFloat(WebMain.songspeedinput.value)
          : 1,
      player1:
        WebMain.player1input.value != "" ? WebMain.player1input.value : "bf",
      player2:
        WebMain.player2input.value != "" ? WebMain.player2input.value : "dad",
      gfVersion:
        WebMain.gfversioninput.value != ""
          ? WebMain.gfversioninput.value
          : "gf",
    };
    var fnfchart = WebMain.curSMFile.makeFNFChart(
      Std.parseInt(WebMain.chartselect.value),
      cfg,
      WebMain.flipchartinput.checked
    );
    var blob = new Blob([JSON.stringify(fnfchart, null, "\t")], {
      type: "text/json",
    });
    var a_elem = window.document.createElement("a");
    a_elem.href = URL.createObjectURL(blob);
    a_elem.download =
      "" + StringTools.replace(cfg.song.toLowerCase(), " ", "-") + ".json";
  ...
  WebMain.main = function () {
    console.log(
      "WebMain.hx:94:",
      "-------Welcome to the SM to JSON converter------"
    );
    WebMain.simfileupload = window.document.getElementById("fileupload");
    WebMain.simfileupload.addEventListener(
      "change",
      WebMain.handleFiles,
      false
    );
    WebMain.generatechartbtn = window.document.getElementById("generate-chart");
    WebMain.generatechartbtn.addEventListener(
      "click",
      WebMain.generateChart,
      false
    );
